...
The implemented functionalities were evaluated via a user study. This section describes the methodology of the study, including the overall setup and design, and presents its results.

== Methodology

This sub-section first describes the goal and high-level design of the user study. It then details the chosen use case and task designs and outlines the collected data. 
...
To assess the helpfulness aspect, participants were given tasks designed to evaluate the impact of the tool on their performance. The aspect of interest was then captured via questionnaires and additional participant feedback.

=== High-Level User Study Design

The evaluation of the functionalities implemented as part of this work was the second part of a larger study conducted in a joint team effort. The first part of the study was concerned with assessing the usability of the system. In particular, participants were asked to model a Zelda dungeon in the tool, including locks and keys. Consequently, participants were already familiarized with the use case context and basic functions of the tool when approaching the tasks described here. Task-specific functionalities (e.g., pathfinding and highlighting, as well as the diagrams) were introduced to the participants prior to the respective tasks.

The general setup for this evaluation was a within-subject or repeated measures study, in which participants completed the tasks once in StatePx and once in another tool. The two task variants necessary for this setup as well as a varying order of tools resulted in the four groups shown in @user-study-groups.
...
The chosen baseline tool was Miro. #todo("why miro?")
...
The target demographic was people with some understanding of game design. Participants were thus mainly recruited in a university lecture from a games engineering study course, though participants also included computer science students with some familiarity with videogames.

The study was conducted exclusively in person. For details on data collection, see @eval-data-collection.

=== Task Designs

The functionalities were evaluated via two distinct tasks: a "Solvability" task, which targeted the functionalities around locks and keys, and a subsequent "Pacing Analysis" task, which focused on the diagram function. Splitting the evaluation into two tasks reduced the context required to understand and complete each task and was an intentional choice to limit the amount of new information participants were confronted with at a time. The "Pacing Analysis" task assumed an understanding of pathfinding including locks and keys and was placed after the "Solvability" task.

For the complete task description including all dungeons and the respective answer keys, see @app-us-design. #todo("add to appendix")

==== Zelda Dungeons from the VGLC dataset

The use cases chosen for this study were dungeons from the Legend of Zelda video game series. #todo("reasoning?") -> ensure realistic tasks.

The exact level designs were extracted from the Video Game Level Corpus #cite(<summervilleVGLCVideoGame2016>). While the dataset provides data from three different Legend of Zelda game, the dungeons for both tasks and all variants were chosen from the same game to ensure consistency in details such as item, lock, or key types and the overall complexity and structure of the dungeons. The dungeons from the game "The Legend of Zelda: Link's Awakening" were best suited for the evaluation tasks. #todo("add reasoning, incl two comparable dungeons per task")

Apart from the full level layout and map of each dungeon, the dataset provides level graphs that are annotated with game-specific concepts. Relevant for the following task designs were the overall graph structure as well as placements of locks, keys (including so-called key items), and enemies, and the locations of start/boss rooms. The graphs included edges modeling visibility, i.e., if room $B$ could be seen from room $A$, an edge connects the corresponding nodes even if the player cannot reach room $B$ from $A$. The concept of visibility, including these edges, was not included in the user study tasks.
...
Some rooms spanned multiple screens (see @multiscreen-example) and were thus modeled with multiple nodes (see @multiscreen-node-numbers) to more accurately reflect the game's pacing or experience. For instance, when passing through one portion (one screen) of a room, any enemies that are in another part of said room and do not spawn or become visible and defeatable until that other part of room is entered are not part of that player's experience.
...
In pix:e, each room was modeled by a node. Any keys, items, and other room data were modeled by the built-in functionalities. This resulting visualization is as described previously in @impl-diagrams and @impl-lk. #todo("refine")

Miro as a general-purpose whiteboard tool offers sticky notes, which can be connected by arrows/lines. They were thus used to represent the individual nodes. Node-specific information was simply written into the sticky notes as text, and locks were added as edge labels. The different lock and key types were encoded as emojis, and an additional sticky note served as a legend (see @miro-legend). This encoding was used to obtain a visualization that is as clear and straightforward as possible given the available options in Miro.
...
To evaluate the analysis functionalities concerning solvability, participants were given an unsolvable dungeon (i.e., a dungeon where the boss room is not reachable from the start room) with multiple issues and asked to identify as many issues as possible. They were introduced to the available lock and key types and the different possible issue types. Before starting the task in pix:e, participants were also given a demonstration of the pathfinding functionality and a legend for the different highlighting colors.

In both tools participants were allowed to use any tool functionalities. In Miro, this for instance includes using the pen tool to draw in paths or marking found issues with text or sticky notes. An example in pix:e apart from the aforementioned pathfinding/highlighting functions would be fixing issues after reporting to perform pathfinding under different conditions.

The dungeons chosen for the two variants of this tasks were the third (LA_3) and fourth (LA_4) dungeons from "Link's Awakening" (LA_3 and LA_4 in #cite(<summervilleVGLCVideoGame2016>)). The main criterion for this choice was the dungeons' size, as they are sufficiently large and thus make the task sufficiently complex while still being feasible manually in Miro. Their node count is around the 75th percentile across all VGLC Zelda data. The two dungeons, being subsequent levels from the same game, have comparable size and complexity (based on the outgoing ranks of nodes) and similar numbers of keys/locks (see @solvability-dungeon-numbers).
...
To make the dungeons unsolvable, 6 issues of varying difficulty were added in each dungeon. For both, this included
...
See @la3-vglc for an impression of the original dungeon layout and @la3-miro for the adapted version with issues in Miro for LA_3. Full-size imagess and pix:e versions are included in @app-s-dungeons.
...
To evaluate the diagram functionality, participants were given a dungeon and a set of questions to answer regarding the distribution and changes in component values throughout the dungeon. Again, participants were introduced to the relevant component types and given a demonstration of the diagrams functionality.
...
For this task, the choice fell on smaller dungeons, in particular the first and second dungeons of "Link's Awakening" (LA_1 and LA_2 in #cite(<summervilleVGLCVideoGame2016>)). Size was again a key criterion, though in contrast to the Solvability task, the dungeons for this task were deliberately selected to have a smaller size (25th percentile in VGLC Zelda data). The intention was to account for expected higher task complexity due to manual pathfinding and calculations in the Miro version. The two levels are again subsequent levels of comparable size and complexity with similar numbers of locks and keys (see @pacing-dungeon-numbers).
...
- adaptations: #todo("add adaptations")
  - structural: #todo("add structural adaptations")
The information provided in the charts included the placement and types locks/keys and, for rooms containing enemies, the enemy count and the time required to beat all enemies. As the VGLC only includes data on whether rooms contain enemies, the completion time and enemy count were based on YouTube walkthroughs (specifically 100% completion walkthroughs to best approximate accurate data for all rooms) #cite(<theretrogamersLegendZeldaLinks2014>), #cite(<theretrogamersLegendZeldaLinks2014a>). Enemies that cannot be beat are ignored in the counts.

The questions given to the participants were designed to cover different aspects of pacing and vary in difficulty. They include comparisons of path sections, the overall distributions throughout the dungeon as well as the relation between the two components:
...
The collected demographics are comprised of participants' age, gender, educational background, occupational background, number of projects/games worked on, previous experience with whiteboard tools and previous experience with game design tools. The data was collected via Google Forms.

Task-related data included, for one, participants' direct responses to the tasks, i.e., any issues participants claimed to have found during the solvability task (with timestamps) and any answers to questions during the pacing analysis task (with timestamps). Additionally, any comments from participants, any clarifications/hints they required and observations about their tool use were logged. This data was logged in a semi-structured live protocol written in Markdown. Moreover, as separate pix:e accounts and Miro boards were used for each participant, any modifications to the charts were also recorded.

After completing all tasks, participants were given the UEQ-S #cite(<schreppDesignEvaluationShort2017>) and a short questionnaire comparing Miro and StatePx (see @res-comparative-questions). Like the demographic data, the reponses were collected via Google Forms.

#todo("disclaimer: questionnaires team effort")

After collection, portions of the data were further processed to enable analysis using Python. Participants' responses to the tasks were manually tagged to obtain total counts of identified issues for the solvability task and total counts of answered questions for the pacing analysis tasks, as well as number of correct responses for both according to the answer key.
...
done in python. data from multiple sources joined into complete dataset via participant IDs.

statistical tests used: Wilcoxon signed-rank test, Mann-Whitney U, Kruskal-Wallis and Dunn's, Levene. reasoning: non-parametric tests (no assumptions made about underlying distributions)
for Wilcoxon and mann-whitney U: two-sided unless specified otherwise. #todo("make sure this is true lol")

== User Study Results

=== Participants

In total, 24 people participated in the study. Ages ranged from 19 to 32 years, with an average of 23.625 years. 15 indicated their gender as male, 5 as female and 4 preferred not to say. #todo("educational/occupational background") 15 participants, or 62.5%, indicated previous experience with whiteboard tools. No participants indicated previous experience with tools specific to game design. #todo("projects")
...
Overall, participants claimed significantly less (Wilcoxon signed-rank test: $T = 48.5$, $p = 0.00564929988211376$) solvability issues in pix:e, see @fig:s-total. The overall answer quality as measured by the F1 score (see @fig:s-f1) is also significantly lower in pix:e (Wilcoxon signed-rank test, $T = 34.0$, $p = 0.0009066055498154164$).
...
In pix:e, participants who indicated previous experience with whiteboard tools tended to claim more issues than participants, see @fig:s-total-by-wb-exp. median 1 for participants without whiteboard experience, 2 for participants with whiteboard experience, Mann-Whitney $U = 96.5$, $p < 0.05$. #todo("effect size"). #todo("difference between performance in Miro"). However, the performance difference between the two tools still remains, as participants with whiteboard tool experience still identified significantly fewer issues in pix:e than in Miro (Wilcoxon signed-rank test: $T = 97.5$, $p < 0.05$).
...
Only 6 participants used any editing functionality in Miro. 3 participants marked issues, 1 participant highlighted valid paths, and 2 participants tracked the available inventory for pathfinding. Of the 6 participants, 5 had indicated previous experience with whiteboard tools. As suggested by @fig:s-total-by-miro-usage and @fig:s-f1-by-miro-usage, there is no obvious impact of Miro tool use on the task completion metrics within Miro. #todo("phrasing") total issues identified: median 2 without Miro usage, 2.5 with Miro usage, Mann-Whitney $U = 57.5$, $p=0.4169872469244896$ #todo("effect size?"). F1: median 0.5 with Miro usage, 0.39285714285714285 without Miro usage, Mann-Whitney $U = 57.5$, $p=0.4169872469244896$ #todo("effect size?")
...
#h(1.8em)
===== First Tool
The no evidence for total number of identified issues (see @fig:s-total-by-first-tool) in either tool differing based on which tool was used first (in pix:e: $U = 56.0$, $p approx 0.176$, in Miro: $U=  61.0$, $p approx 0.525$). Also, participants still claimed significantly more issues in Miro than pix:e, regardless of which tool was used first (Miro first: $U = 113.5$, $p approx 0.007$, pix:e first: $U=109.5$, $p approx 0.014$).
However, for the F1 scores (see @fig:s-f1-by-first-tool), the tool order does seem to make a difference: The pix:e-first group achieved higher median F1 score in both tools, with the difference being significant according to the Mann-Whitney U test (in Miro: $U = 34.5$, $p approx 0.013$, in pix:e: $U = 36.0$, $p approx 0.018$). #todo("speculate why?") The difference in F1 scores between the two tools for the pix:e-first group remains significant ($U = 120.5$, $p approx 0.002$).
...
There is a slight trend towards a higher median in total and correct answers in pix:e, but overall the distribution skews higher for miro. #todo("interpret?") #todo("adjust image zoom?")
...
#h(1.8em)
===== Impact of Whiteboard Tool Experience
Within pix:e, participants with whiteboard experience tended to answer significantly more questions (see @fig:p-total-by-wb-exp): the median 4 with whiteboard experience, 3 without whiteboard experience, Mann-Whitney $U = 103.5$, $p approx 0.015$. They also gave significantly more correct answers (see @fig:p-right-by-wb-exp): median 3 with whiteboard experience, 2 without, Mann-Whitney $U = 103.0$, $p approx 0.017$.
- slight skews in median/distribution: number of answered questions higher in pix:e than miro for participants with whiteboard experience, total number and number of correct answers lower in pix:e than Miro for people without whiteboard experience. however, differences in distribution do not seem to be statistically significant (Wilcoxon signed-rank test: $T = 38.0$, $p approx 0.326$ for total answers in pix:e vs Miro from participants with Whiteboard experience, $T = 15.0$, $p approx 0.207$ for total answers from participants without whiteboard experience)
...
In Miro, however, there is a significant difference in distributions for both the total questions answered ($H = 12.251$, $p approx 0.007$) and correctly answered questions ($h = 10.571$, $p approx 0.014$). Dunn's post-hoc test reveals that specifically the difference between the "BM" and "BP" groups is significant in both Miro ($p approx 0.004$) and pix:e ($p approx 0.008$). A possible explanation for this contrast is that as task B is apparently more challenging (see @res-p-a-vs-b), when encountering it as a first task (as participants of the B-groups did), the previously (@res-p-task-completion) mentioned slight difference between the tools may be amplified.
...
#h(1.8em)
===== First Task
The overall distributions of task completion metrics in Miro do not differ significantly (Mann-Whitney $U = 72.0$, $p approx 1.0$ for total answers, $U = 75.5$, $p approx 0.859$ for right answers). There is a significant difference in variance in both metrics for performance in Miro in participants who encountered task variant B first (Levene's test $W approx 4.313$, $p approx 0.0497$), which is expected from the aforementioned large difference in results between the B-first groups (see @res-p-a-vs-b).
...
Each item is a pair of opposing adjectives, and participants were asked to place StatePx on a 7-point scale between the adjectives. This translates to a numeric scale ranging from -3 to 3. 
The responses to the UEQ-S questionnaire were evaluated using the analysis tool provided by the authors #cite(<hinderksBenchmarkShortVersion2018>). It provides mean scores for each item and rates the pragmatic, hedonic, and overall score against a benchmark. #todo("what is the benchmark?")
...
Answers were given on a 5-point Likert scale, ranging from fully disagree (1) to fully agree (5). @fig:cq-histogram shows the answer distributions for each question.
...
Overall, participants indicated a preference for StatePx over Miro, though answers were mixed for the first question(s) in particular. This result is in line with the high hedonic quality indicated by the UEQ-S.

A higher score variance/split is observed in participants with whiteboard tool experience in the first three questions, see @app-cq-by-wb-exp. For last question however, participants who indicated previous experience with whiteboard tools indicated a particularly strong preference for pix:e #todo("back this up"). Answers for the first three questions may be divided because on one hand, these participants may be more used to a Miro-like UI and thus more comfortable using Miro, but on the other hand, they may also be better equipped to recognize the advantage of a dedicated tool compared to a generic whiteboard tool.
...
- addition to data model: `symbol` for both locks and keys, can be selected in picker when creating definitions. defaults: 🔒 and 🔑.
    ...
    - edges show symbols of all assigned locks as label, including their multiplicity. tooltip more involved in current framework. instead: legend popover
...
- 6 participants reported low contrast between nodes and background as a usability issue, especially when zoomed out, see @feedback-written
